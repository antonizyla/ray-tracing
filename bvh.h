...
public:
    ...
    bvh_node(std::vector<std::shared_ptr<hittable>> &src_objs, size_t start, size_t end, double t0, double t1);
    ...
    static inline bool box_compare(const std::shared_ptr<hittable> a, const std::shared_ptr<hittable> b, int axis) {
        aabb box_a;
        aabb box_b;
...
bvh_node::bvh_node(const std::vector<std::shared_ptr<hittable>> &src_objects, size_t start, size_t end, double time0,
    ...
    if (object_span == 1) {
        ...
        unsigned long mid = start + object_span / 2;
